...
contract USDPool is ReentrancyGuard, Pausable, Ownable {
    ...
    function exchange(address _tokenIn, uint256 _amountIn) external nonReentrant whenNotPaused {
    ...
    function redeem(address _tokenOut, uint256 _amount, address _receiver) external nonReentrant {
    ...
    function exchangeNative() external payable nonReentrant whenNotPaused {
    ...
    function redeemNative(uint256 _amount) external payable nonReentrant {
    ...
    function setNativeExchange(bool _isNativeExchange) external onlyOwner {
